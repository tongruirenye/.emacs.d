...
(defcustom org-drill-card-type-alist
  '((nil org-drill-present-simple-card)
    ...
    ("dictation" org-drill-present-simple-dictation-card)
    ("conjugate"
  ...
  (let ((ch nil)
        ...
        (typed-answer-statement (if drill-typed-answer
                                    (format "Your answer: %s\n"
                                            drill-typed-answer)
                                  ""))
        (key-prompt (format "(0-5, %c=help, %c=edit, %c=tags, %c=audio, %c=quit)"
                            org-drill--help-key
                            org-drill--edit-key
                            org-drill--tags-key
			    ?a
...
%sHow well did you do? %s"
	...
	(if (and (stringp input) (eql ?a (elt input 0)))
	    (org-drill--show-audio))
...
Consider reformulating the item to make it easier to remember.\n"
                                       'face '(:foreground "red"))
                           full-prompt)))
    (while (memq ch '(nil org-drill--tags-key ?a))
      (setq ch nil)
...
(defun org-drill-present-simple-dictation-card ()
  (with-hidden-comments
   (with-hidden-cloze-hints
    (with-hidden-cloze-text
     (org-drill-hide-all-subheadings-except nil)
